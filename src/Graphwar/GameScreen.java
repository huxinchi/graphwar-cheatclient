...
public class GameScreen extends JPanel implements ActionListener, StartStopPanel, KeyListener, MouseListener
{
	...
	private FunctionPresetsWindow functionPresetsWindow;
	
	private javax.swing.JButton historyButton;
	private javax.swing.JButton functionPresetsButton;
	private javax.swing.JButton playerCoordinatesButton;
	private javax.swing.JButton hideTypingButton;
	private javax.swing.JButton typingModeButton;

	private boolean suppressInputEvents = false;
	
	private double heightDifference = Double.NaN;
	private boolean showHeightDifference = false;
	...
	{		
		...
		/////	Main Screen		/////////////
			...
			plane.addMouseMotionListener(new java.awt.event.MouseMotionAdapter() {
				public void mouseMoved(java.awt.event.MouseEvent e) {
					updateHeightDifference(e.getX(), e.getY());
				}
				public void mouseExited(java.awt.event.MouseEvent e) {
					showHeightDifference = false;
					repaint();
				}
			});
			...
			functionPresets = new FunctionPresets();
			functionHistory = new FunctionHistory();
			
		functionPresetsButton = new javax.swing.JButton("函数");
		functionPresetsButton.setBounds(15, 620, 80, 30);
		functionPresetsButton.addActionListener(this);
		this.add(functionPresetsButton);

		historyButton = new javax.swing.JButton("历史记录");
		historyButton.setBounds(105, 620, 100, 30);
			historyButton.addActionListener(this);
			this.add(historyButton);
			
			playerCoordinatesButton = new javax.swing.JButton("玩家坐标: " + (Plus.isShowPlayerCoordinates() ? "ON" : "OFF"));
		playerCoordinatesButton.setBounds(220, 620, 140, 30);
			playerCoordinatesButton.addActionListener(this);
			this.add(playerCoordinatesButton);
			
			hideTypingButton = new javax.swing.JButton("隐藏输入: " + (Plus.hideTyping ? "ON" : "OFF"));
		hideTypingButton.setBounds(375, 620, 140, 30);
			hideTypingButton.addActionListener(this);
			this.add(hideTypingButton);
			
			typingModeButton = new javax.swing.JButton("*模式: 转换");
		typingModeButton.setBounds(530, 620, 120, 30);
			typingModeButton.addActionListener(this);
			typingModeButton.setVisible(Plus.hideTyping);
			this.add(typingModeButton);
	...
	public void paint(Graphics g)
	{
		...
		if(showHeightDifference && !Double.isNaN(heightDifference)) {
			g.setFont(new Font("Sans", Font.PLAIN, 12));
			g.setColor(Color.BLACK);
			String text = String.format("高度差 = %.2f", heightDifference);
			g.drawString(text, 15, 680);
		}
	}
	...
	public void refreshFunction()
	{
		...
		if(player.isLocalPlayer() && !(player instanceof ComputerPlayer))
		{
			...
			for(String historyItem : functionHistory.getAll()) {
				String expandedHistory = functionPresets.expandFunction(historyItem);
				if(expandedHistory.equals(function)) {
					displayFunction = historyItem;
					break;
				}
			}
			
			final String finalFunction = displayFunction;
			...
			new Runnable() 
			{
				public void run()
				{
					...
					funcField.setText(finalFunction);
				}
			}
			);
		}
	...
	else if(arg0.getSource()==functionPresetsButton)
		{
			showFunctionPresets();
		}
			else if(arg0.getSource()==historyButton)
			{
				showHistory();
			}
			else if(arg0.getSource()==playerCoordinatesButton)
			{
				Plus.setShowPlayerCoordinates(!Plus.isShowPlayerCoordinates());
				playerCoordinatesButton.setText("玩家坐标: " + (Plus.isShowPlayerCoordinates() ? "ON" : "OFF"));
				this.repaint();
			}
			else if(arg0.getSource()==hideTypingButton)
			{
				Plus.hideTyping = !Plus.hideTyping;
				hideTypingButton.setText("隐藏输入: " + (Plus.hideTyping ? "ON" : "OFF"));
				typingModeButton.setVisible(Plus.hideTyping);
			}
			else if(arg0.getSource()==typingModeButton)
			{
				if(Plus.typingMode.equals("expand"))
				{
					String customText = javax.swing.JOptionPane.showInputDialog(this, "你输入的内容将在他人视角中您的函数输入框中显示:", "自定义输入框展示文本", javax.swing.JOptionPane.PLAIN_MESSAGE);
					if(customText != null && !customText.isEmpty())
					{
						Plus.typingMode = "text";
						Plus.typingTextPreview = customText;
						typingModeButton.setText("*模式: 自定义");
					}
				}
				else
				{
					Plus.typingMode = "expand";
					typingModeButton.setText("*模式: 转换");
				}
			}
		}
	}

	private void updateHeightDifference(int screenX, int screenY) {
		showHeightDifference = false;
		...
		Soldier mySoldier = currentPlayer.getSoldiers()[0];
		if(mySoldier == null || !mySoldier.isAlive()) {
			repaint();
			return;
		}
		double myY = mySoldier.getY();
		
		for(Player player : graphwar.getGameData().getPlayers()) {
			for(Soldier soldier : player.getSoldiers()) {
				...
				int soldierScreenX = (int)(soldierX * Constants.PLANE_LENGTH / Constants.PLANE_GAME_LENGTH + Constants.PLANE_LENGTH / 2);
				int soldierScreenY = (int)(-soldierY * Constants.PLANE_LENGTH / Constants.PLANE_GAME_LENGTH + Constants.PLANE_HEIGHT / 2);
				
				int distance = (int)Math.sqrt(Math.pow(screenX - soldierScreenX, 2) + Math.pow(screenY - soldierScreenY, 2));
				if(distance < 20) {
					heightDifference = myY - soldierY;
	...
	public void updateFunction(String function)
	{
		...
		// TO1DO: 这里需要根据Plus的预测功能来判断是否需要预览
		graphwar.getGameData().previewFunctionLocal(function);
	}
	...
	public void setPreviewFunction(Function func)
	{
		this.plane.setPreviewFunction(func);
		this.plane.repaint();
	}
	
	public void clearPreviewFunction()
	{
		this.plane.clearPreviewFunction();
		this.plane.repaint();
	}
	...
	{		
		...
		{
//			Plus.setPredictionEnabled(!Plus.isPredictionEnabled());
			if(!Plus.isPredictionEnabled())
			{
				clearPreviewFunction();
			}
			return;
		...
		if(Plus.hideTyping)
		{
			if (Plus.typingMode.equals("text"))
			{
				graphwar.getGameData().sendFunctionPreview(Plus.typingTextPreview);
			} else if (Plus.typingMode.equals("expand"))
			{
				graphwar.getGameData().sendFunctionPreview(functionPresets.expandFunction(function));
			}
		} else
		{
	...
	{		
		...
		{
			String function = funcField.getText();
			function = functionPresets.expandFunction(function);
			graphwar.getGameData().previewFunctionLocal(function);
		}
	}
	...
	public void mouseEntered(MouseEvent arg0) {
		// TO1DO.txt Auto-generated method stub
	...
	public void setFunctionText(String text) {
		funcField.setText(text);
		funcField.requestFocus();
	}
	...
	public void setFunctionTextAndPreview(String text) {
		funcField.setText(text);
		String expanded = functionPresets.expandFunction(text);
		graphwar.getGameData().previewFunctionLocal(expanded);
	}
	
	public void addToHistory(String function) {
		functionHistory.add(function);
	}
	
	public void showHistory() {
		if (historyWindow == null) {
			historyWindow = new HistoryWindow(this, functionHistory);
		}
		historyWindow.showWindow();
	}
	
	public FunctionHistory getFunctionHistory() {
		return functionHistory;
	}


	public void showFunctionPresets() {
		if (functionPresetsWindow == null) {
			functionPresetsWindow = new FunctionPresetsWindow(this, functionPresets);
		}
		functionPresetsWindow.showWindow();
	}
}