...
public class MainMenuScreen extends JPanel implements ActionListener
{
	...
	private boolean isGlobalJoinMode;
	...
	{		
		...
		new Runnable()
		{
			public void run()
			{
				...
				if(isGlobalJoinMode)
				{
					ipFieldJoin.setText(Constants.GLOBAL_IP);
					portFieldJoin.setText(Constants.GLOBAL_PORT+"");
				}
				else
				{
					ipFieldJoin.setText("");
	...
	private void paintPlusPlus(Graphics g)
	{
		...
		if (Plus.canJoinGlobal)
		{
			plusPlus = "注：这个版本是 Lzdqesj 修改的作弊客户端 (允许加入Global)";
		} else {
			plusPlus = "注：这个版本是 Lzdqesj 修改的作弊客户端 (不允许加入Global)";
		}
		FontMetrics fontMetrics = Toolkit.getDefaultToolkit().getFontMetrics(font);
		int textWidth = fontMetrics.stringWidth(plusPlus);
		int x = (Constants.WIDTH - textWidth) / 2;
		int y = 275;
		
		g.drawString(plusPlus, x, y);
	}
			...
			else if(arg0.getSource()==this.yesButtonGlobal || arg0.getSource()==nameFieldGlobal)
			{
				...
				if(name!=null && name.length()>0)
				{
					...
					else
					{
						...
						{
							this.showMessage("连接失败: " + e.getMessage());
				...
				try
				{
					...
					if(name!=null && name.length()!=0)
					{
						...
						else
						{
							...
							{
								this.showMessage("创建失败! 请检查端口号是否已经被占用。");
			...
			else if(arg0.getSource()==yesButtonJoin || arg0.getSource()==nameFieldJoin || arg0.getSource()==ipFieldJoin || arg0.getSource()==portFieldJoin)
			{
				try
				{
					...
					if(name!=null && name.length()>0 && ip!=null && ip.length()>0)
					{
						...
						{
							if(this.isGlobalJoinMode)
							{
								boolean usingGlobalDefaults = ip.trim().equalsIgnoreCase(Constants.GLOBAL_IP.trim()) && port == Constants.GLOBAL_PORT;

								if(!Plus.canJoinGlobal && usingGlobalDefaults)
								{
									// 禁止使用默认 Global 地址 —— 但可以手动修改 IP/端口
									this.showMessage("臭外挂，你根本就不配在 Global 服务器里霍霍别人！");
									this.repaint();
									return;
								}

								// 修改 Constants
								Constants.GLOBAL_IP = ip;
								Constants.GLOBAL_PORT = port;
