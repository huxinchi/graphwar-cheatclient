...
public class GameData implements Runnable
{
	...
	public void previewFunctionLocal(String functionString)
	{
		Player currentPlayer = getCurrentTurnPlayer();

		if(currentPlayer.isLocalPlayer() && drawingFunction == false && functionString != null && functionString.length() > 0)
		{
			previewFunctionForPlayer(functionString, currentPlayer);
		}
		else
		{
			((GameScreen)graphwar.getUI().getScreen(Constants.GAME_SCREEN)).clearPreviewFunction();
		}
	}

	public void previewFunctionForPlayer(String functionString, Player player)
	{
		if(drawingFunction || functionString == null || functionString.length() == 0 || player == null)
		{
			((GameScreen)graphwar.getUI().getScreen(Constants.GAME_SCREEN)).clearPreviewFunction();
			return;
		}
		...
		try
		{
			Function previewFunc = new Function(functionString);

			Player[] playersArray = players.toArray(new Player[0]);
			boolean inverted = (player.getTeam() == Constants.TEAM2);

			switch(gameMode)
			{
				case Constants.NORMAL_FUNC:
					previewFunc.processFunctionRange(obstacle, playersArray, players.size(), currentTurn, inverted);
					break;
				case Constants.FST_ODE:
					previewFunc.processRK4Range(obstacle, playersArray, players.size(), currentTurn, inverted);
					break;
				case Constants.SND_ODE:
					previewFunc.processRK42Range(obstacle, playersArray, players.size(), currentTurn, player.getCurrentTurnSoldier().getAngle(), inverted);
					break;
			}

			((GameScreen)graphwar.getUI().getScreen(Constants.GAME_SCREEN)).setPreviewFunction(previewFunc);
		}
	...
	public void sendFunction(String function)
	{
		...
		if(currentPlayer.isLocalPlayer() && drawingFunction == false)
		{
			String expandedFunction = functionPresets.expandFunction(function);
			
			try 
			{
				@SuppressWarnings("unused")
				Function func = new Function(expandedFunction);
			} 
			catch (MalformedFunction e)
			{
			...
			try 
			{
				String message = NetworkProtocol.FIRE_FUNC+"&"+currentPlayer.getID()+"&"+URLEncoder.encode(expandedFunction, "UTF-8");
				serverConnection.sendMessage(message);
				
				GameScreen gameScreen = (GameScreen)graphwar.getUI().getScreen(Constants.GAME_SCREEN);
				gameScreen.addToHistory(function);
			} 
			catch (UnsupportedEncodingException e) 
			{
				e.printStackTrace();
	...
	public void addPC(String name, int level)
	{
		if(players.size() < Constants.MAX_PLAYERS)
		{
            // noinspection removal,CachedNumberConstructorCall,UnnecessaryBoxing
	...
	private void removePlayerMessage(String[] info) throws Exception
	{
		...
		((PreGameScreen)graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN)).addChat(null,player.getName()+" 离开了游戏");
		...
		if(gameState == Constants.GAME)
		{
			...
			((GameScreen)graphwar.getUI().getScreen(Constants.GAME_SCREEN)).addChat(null,player.getName()+" 离开了游戏。");
	...
	private void addPlayerMessage(String[] info) throws Exception 
	{
		...
		((PreGameScreen)graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN)).addChat(null,player.getName()+" 加入了游戏。");
	...
	private void updateFunctionMessage(String[] info) throws Exception
	{
		if(info.length == 3 && drawingFunction == false)
		{
			...
			if(players.get(currentTurn).getID() == playerID)
			{
				...
				if(currentPlayer.isLocalPlayer() == false)
				{
					previewFunctionForPlayer(function, player);
				}
				else
				{
					((GameScreen)graphwar.getUI().getScreen(Constants.GAME_SCREEN)).clearPreviewFunction();
	...
	private void finishGameMessage(String[] info) throws Exception
	{
    	...
    	while(itr.hasNext())
    	{
    		...
    		for(int i=0; i<soldiers.length; i++)
    		{
    			...
    			{
    				((PreGameScreen)graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN)).addChat(null, player.getName()+" 获得了游戏胜利。");
        				break;
    			}
    		}
	...
	private void setLeaderMessage(String[] info) throws Exception
	{
		...
		((PreGameScreen)graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN)).addChat(null, "您现在是房主。");
	}
	...
	private class Countdowner implements Runnable
	{
		...
		public void run() 
		{
			// *********************
			countingDown = true;
			gameData.displaySystemMessage("游戏即将开始 ...");
			gameData.displaySystemMessage("具体等待时间按服务端设置, 反正客户端是秒开始。");
			
			for(int i=Constants.START_GAME_DELAY/1000; i>0; i--)
			{
				if(countingDown)
				{
					gameData.displaySystemMessage("游戏即将在 " + i +" 秒后开始 ...");
				}
				else
				{
					break;
	...
	public void kickFromGame()
	{
		...
		{
			((PreGameScreen)graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN)).showMessage("你已断开连接。");
		}
		else if(this.gameState == Constants.GAME)
		{
			((GameScreen)graphwar.getUI().getScreen(Constants.GAME_SCREEN)).showMessage("你已断开连接。");
			((GameScreen)graphwar.getUI().getScreen(Constants.GAME_SCREEN)).stopPanel();
		}
